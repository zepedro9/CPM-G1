...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (!isValidExpirationCard(req.body.card.expirationDate)) 
            return res.status(403).send({ "message": "Invalid credit card expiration date" });
...
router.post('/update-key', async (req, res) => {
    try {
        if (!req.body.pk || !req.body.uuid) {
            return res.status(200).send({ message: "Missing body parameters" });
        }
        await User.updateOne({ _id: req.body.uuid }, { pk: req.body.pk });
