...
const val PRODUCTS_ADDRESS: String =
    "http://" + ConfigHTTP.BASE_ADDRESS + ":3000/api/basket/products?ids="
...
class BasketFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Init Basket
        if (basketIdsList.isNotEmpty()) {
            Thread(InitBasket(PRODUCTS_ADDRESS + basketIdsList, this::initBasket)).start()
        } else {
            initView()
        }
    }
    ...
    inner class BasketAdapter() :
        ArrayAdapter<BasketItem>(this.requireContext(), product_row, basket) {
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            val row = convertView ?: (requireContext() as AppCompatActivity).layoutInflater.inflate(
                product_row,
                parent,
                false
            )
